 ...
 */
@Api(tags = "spu属性值; InnoDB free: 5120 kB 管理")
@RestController
...
public class ProductAttrValueController {
    ...
    public Resp<List<ProductAttrValueEntity>> findProBySpuId(@PathVariable("spuId")Long spuId){
        List<ProductAttrValueEntity> valueEntities = this.productAttrValueService.findProBySpuId(spuId);

        return Resp.ok(valueEntities);
    }
    ...
    public Resp<PageVo> list(QueryCondition queryCondition) {
        PageVo page =
                productAttrValueService.queryPage(queryCondition);
    ...
    public Resp<ProductAttrValueEntity> info(@PathVariable("id") Long id) {
        ProductAttrValueEntity productAttrValue =
                productAttrValueService.getById(id);
