...
public class MemberServiceImpl extends ServiceImpl<MemberDao, MemberEntity> implements MemberService {
    ...
    @Override
    public Boolean queryData(String data, Integer type) {
    ...
    @Override
    public void registry(MemberEntity memberEntity, String code) {
        //验证手机验证码

        //生成盐
        String substring = UUID.randomUUID().toString().substring(0, 6);
        memberEntity.setSalt(substring);

        //盐加密
        memberEntity.setPassword(DigestUtils.md5Hex(memberEntity.getPassword()+substring));
        //新增用户
    ...
    public MemberEntity queryUser(String username, String password) {
        ...
        if (!StringUtils.equals(password, memberEntity.getPassword())){
            throw new MemberException("用户密码错误！");
        }
